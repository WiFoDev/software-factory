...
**v0.0.9 ships status-aware run-sequence + per-spec timeout + scaffold scripts + dep-aware internal-consistency** — `run-sequence` now skips `status: drafting` specs by default (with `--include-drafting` to opt in); a per-spec `agent-timeout-ms` frontmatter field overrides the 600s ceiling for wide-blast-radius specs; the scaffold's `package.json` ships `scripts: { typecheck, test, check, build }` matching every DoD claim; the `internal-consistency` reviewer judge follows `depends-on` edges so shared constraints in multi-spec products are no longer flagged as unreferenced. v0.0.1 framework, v0.0.2 single-shot agent, v0.0.3 closed loop, v0.0.4 spec quality + bootstrap, v0.0.5 publish + implement-guidelines, v0.0.6 v0.0.5.x cluster, v0.0.7 real-product workflow, v0.0.8 discoverability, **v0.0.9 status-aware run-sequence + per-spec timeout + scaffold scripts + dep-aware internal-consistency**.
