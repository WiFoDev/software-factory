...
**v0.0.8 ships discoverability + baseline reset** — `factory init` now drops `/scope-project` into `.claude/commands/` zero-config, and the scaffold README documents the multi-spec flow (`/scope-project` → `factory spec lint` → `factory-runtime run-sequence`). A fresh-repo agent reading just the scaffolded README has all the signal it needs to ship a multi-spec product. v0.0.7 BASELINE quantified the gap (the v0.0.7 deliverables were on npm but invisible to fresh-repo agents); v0.0.8 closes it. v0.0.1 framework, v0.0.2 single-shot agent, v0.0.3 closed loop, v0.0.4 spec quality + bootstrap, v0.0.5 publish + implement-guidelines, v0.0.6 v0.0.5.x cluster, v0.0.7 real-product workflow, **v0.0.8 discoverability**.
