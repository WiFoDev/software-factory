...
- `claude` CLI is invoked headless via `claude -p ... --bare --output-format json`. Subscription auth is implicit (no `ANTHROPIC_API_KEY` is read or set by the runtime). The chosen output format gives a single JSON envelope on stdout: `usage.input_tokens` for the cost cap, `is_error` for self-reported failure, `result` for the final text. The `--bare` flag suppresses the CLI's banner/preamble noise so JSON parsing is clean.
...
`RuntimeErrorCode` (existing union) gains two members:
