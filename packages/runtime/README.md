...
- **`run-sequence` skips already-converged specs (scoped to specsDir).** Before invoking each spec's `run()`, the sequence-runner queries the context store for prior `factory-run` records whose `payload.specId` matches and whose `parents[]` includes a `factory-sequence` whose `payload.specsDir` matches the CURRENT sequence's specsDir (resolved absolute path). Match → skip + emit `factory-runtime: <id> already converged in run <runId> — skipping` to stdout; the `SequenceReport.specs[]` entry has `status: 'converged'` and `runReport.runId` points at the PRE-EXISTING factory-run (no fresh run is performed). Re-running the same specs against a different `<dir>` runs them fresh — the dedup is scoped, not global.
- **`run-sequence`'s `buildDag` resolves `depends-on` against `<dir>/done/`.** The OPTIONAL `<specsDir>/done/*.md` walk (non-recursive) returns a `donePool` of dep-context-only specs that are NOT executed. Dep-id existence is validated against `included ∪ donePool`; cycle detection runs over the same union; topological execution order contains ONLY `included` ids. Moving a shipped spec to `done/` no longer breaks the next sequence's `depends-on` resolution.
- **New built-in phase: `dodPhase`.** Reads each spec's `## Definition of Done` section, classifies each bullet as `kind: 'shell'` or `kind: 'judge'` via the new `parseDodBullets` helper from `@wifo/factory-core`, runs shell bullets via Bash from the run's `cwd`, and dispatches non-shell bullets to the harness `JudgeClient`. Persists a new `factory-dod-report` record per iteration. Convergence requires all three phases to pass.
- **Default graph is now `[implement, validate, dod]`** with edges `[['implement', 'validate'], ['validate', 'dod']]`. `--no-implement` keeps `dod` (graph becomes `[validate, dod]`) — the maintainer "I implemented by hand; verify the work" path. Pass `--skip-dod-phase` (or set `factory.config.json runtime.skipDodPhase: true`) to drop `dod` from the graph; CLI flag wins over config wins over the built-in default `false`. `--no-implement --skip-dod-phase` restores the v0.0.1 `[validate]`-only graph for back-compat.
- **Shell allowlist (locked).** A bullet qualifies as `shell` ONLY when its body contains exactly one backtick-wrapped token whose first word is one of `pnpm`, `bun`, `npm`, `node`, `tsc`, `git`, `npx`, `bash`, `sh`, `make`, `pwd`, `ls`, OR begins with `./` / `../` (relative-path script). Multiple backticks → `judge`. Zero backticks → `judge`. Anything else → `judge`. Conservative by design: prevents accidental shell injection from prose mishaps.
- **Per-bullet timeout default: 60_000 ms** (`DodPhaseOptions.timeoutMs`). Distinct from `RunOptions.maxAgentTimeoutMs` (which gates `claude -p` subprocesses). On timeout, the bullet reports `status: 'error'`, `exitCode: null`, `stderrTail: 'dod-timeout (after Nms)'`. `stderrTail` is capped at **2 KB** per bullet with a `[runtime] truncated stderr` marker.
- **Cross-iteration `# Prior DoD report` prompt section.** When iteration N's `dodPhase` fails, iteration N+1's `implementPhase` prompt grows a new `# Prior DoD report` section listing failed bullets in the locked format `**\`<command>\`** — exit <code>: <stderrTail>`. Same per-line + section-total caps as v0.0.3's `# Prior validate report` (1 KB per line, 50 KB section, byte-stable across iterations of the same failure for cache friendliness). Truncation marker: `[runtime] truncated prior-DoD section`. Section position: AFTER `# Prior validate report`, BEFORE `# Working directory`.
- **Public API surface deltas.** `@wifo/factory-runtime/src/index.ts`: 21 → **23** names (+`dodPhase`, +`DodPhaseOptions`). `@wifo/factory-core/src/index.ts`: 29 → **31** names (+`parseDodBullets`, +`DodBullet`). `RunOptions` gains an optional `skipDodPhase?: boolean` field. `RuntimeErrorCode` is unchanged (still 14 values from v0.0.9; DoD failures land as `'fail'` or `'error'` phase status, not new error codes).
- **Explicitly NOT shipping in v0.0.10**: parallel DoD bullet execution (sequential by design), per-bullet result caching (re-runs every iteration), single-bullet retry-on-fail (one attempt per iteration), or user-extensible runner allowlist (locked at the canonical set; new runners require a point release).
