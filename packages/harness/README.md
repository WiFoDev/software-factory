...
**Per-scenario coverage carve-out (v0.0.12+).** When a `test:` line carries a pattern (`-t "<name>"`), the harness adds `--coverage=false` to the `bun test` invocation. Coverage is a *holistic* property — meaningful only when the whole suite runs — so asserting coverage on a single filtered scenario is a category error: a host repo's `bunfig.toml` coverage threshold trips on the tiny slice and the runner reports `validate-fail` on correct work. Holistic test runs (no pattern) preserve the host's coverage configuration. The host's coverage gate runs separately at DoD time on the full suite.
